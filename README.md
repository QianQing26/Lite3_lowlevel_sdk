...
│  ├ RLHandshake  — 等待 NX 发送 HEARTBEAT_READY     │
...
```
Idle ──→ StandUp ──→ RLHandshake ──→ RLControl ──→ JointDamping
          ↑ 超时 / 姿态异常 / 用户指令  │
          └──────────────────────────┘
                       │
                       ↓ (3 s 后)
...
SDK 与上层控制器通过 **UDP 二进制协议** 通信。协议定义见 `communication/nx_network_codes.hpp`。上层控制器应使用 [Lite3 Highlevel SDK](https://github.com/DeepRoboticsLab/Lite3_highlevel_sdk)，它提供了 Python 和 C++ 两套 API，内置握手协议、观测构建、指令处理和 ONNX Runtime 推理。
...
Lite3_lowlevel_sdk/
  ...
  state_machine/          # 状态机 (Idle / StandUp / RLHandshake / RLControl / JointDamping)
  interface/              # 硬件接口 + 手柄接口
...
> 上层控制 API 请使用 [Lite3 Highlevel SDK](https://github.com/DeepRoboticsLab/Lite3_highlevel_sdk)。
...
- 新增上层通信协议二进制规范，配套 [Lite3 Highlevel SDK](https://github.com/DeepRoboticsLab/Lite3_highlevel_sdk)
